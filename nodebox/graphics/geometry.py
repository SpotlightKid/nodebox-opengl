...
try:
    ...
    from nodebox.ext.geometry import angle, distance, coordinates, rotate
except:
    ...
    def coordinates(x0, y0, distance, angle):
        """Return the location of a point by rotating around origin (x0,y0)."""
        return (x0 + cos(radians(angle)) * distance,
                y0 + sin(radians(angle)) * distance)


    def rotate(x, y, x0, y0, angle):
        """Return the coordinates of (x,y) rotated around origin (x0,y0)."""
        x, y = x-x0, y-y0
...
def reflect(x, y, x0, y0, d=1.0, a=180):
    """Return the reflection of a point through origin (x0,y0)."""
...
def lerp(a, b, t):
    """Return linear interpolation between a and b for time t between 0.0-1.0.
...
def bounce(x):
    """Return a bouncing value between 0.0 and 1.0 for a value between 0.0-1.0.
    """
    return abs(sin(2 * pi * (x + 1) * (x + 1)) * (1 - x))


def clamp(v, a=0.0, b=1.0):
    """Clamp value to between a and b (inlusive)."""
    if a > b:
        a, b, = b, a
...
# Fast C implementations:
try:
    from nodebox.ext.geometry import smoothstep
except:
    def smoothstep(a, b, x):
        """Return a smooth transition between 0.0 and 1.0 using Hermite
...
def line_line_intersection(x1, y1, x2, y2, x3, y3, x4, y4, infinite=False):
    ...
    If infinite=False the points specify two finite line segments .
...
def circle_line_intersection(cx, cy, radius, x1, y1, x2, y2, infinite=False):
    """Return a list of points where the circle and the line intersect.
...
def point_in_polygon(points, x, y):
    ...
    The algorithm does not always report correctly when the point is very close
    to the boundary. The polygon is passed as a list of (x,y)-tuples.
...
try:
    ...
    from nodebox.ext.geometry import superformula
except:
    def superformula(m, n1, n2, n3, phi):
        """A generalization of the superellipse first proposed by Johan Gielis.

        It can be used to describe many complex shapes and curves that are
        found in nature.
...
class AffineTransform:
    ...
    def _mmult(self, a, b):
        """Return the 3x3 matrix multiplication of A and B.
    ...
    def rotation(self):
        # 0.0 => 360.0
        return (degrees(atan2(self.matrix[1], self.matrix[0])) + 360) % 360
    ...
    def transform_point(self, x, y):
        """Return the new coordinates of (x,y) after transformation."""
        m = self.matrix
    ...
    def transform_path(self, path):
        """Return a BezierPath object with the transformation applied."""
...
class Bounds:
    ...
    def intersection(self, b):
        """Return bounds that encompass the intersection of the two.
    ...
    def union(self, b):
        """Return bounds that encompass the union of the two."""
    ...
    def contains(self, *a):
        """Return True if the given point or rectangle falls within the bounds.
        """

...
class PerlinNoise:
    ...
    def __init__(self, permutation=None):
        ...
        You can optionally pass a permutation pattern, which is a list of 256
    ...
    def generate(self, x, y=0, z=0):
        """Return a smooth value between -1.0 and 1.0.

        The x, y, z parameters determine the coordinates in the noise
        landscape.

        Since the landscape is infinite, the actual value of a coordinate
        doesn't matter, only the distance between successive steps. The smaller
        the difference between steps, the smoother the noise sequence. Steps
        between 0.005 and 0.1 usually work best.
